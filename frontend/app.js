...
async function carregarProdutos() {
  ...
  try {
    ...
    const token = localStorage.getItem('token');
    if (!token) {
      throw new Error('Token nao encontrado. Faca login no sistema primeiro.');
    }

    const url = termo
      ? `${API_BASE}/api/produtos?busca=${encodeURIComponent(termo)}`
    ...
    produtos.forEach((produto) => {
      ...
      clone.querySelector('.produto-estoque').textContent = `Estoque: ${produto.estoque ?? produto.estoque_atual ?? '-'}`;
